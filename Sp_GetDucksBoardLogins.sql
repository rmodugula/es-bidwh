...
/****** Object:  View [dbo].[GetDucksBoardLogins]    Script Date: 1/2/2015 9:58:09 AM ******/
SET ANSI_NULLS ON
GO
...
and MONTH=case when (case when month(getdate())=1 then 12 else MONTH(getdate())-1 end)=(select Month from (select *,row_number() over (order by id desc) as row 
from fillhub.dbo.InvoiceMonth)Q where ROW=1) then MONTH(getdate()) else (case when month(getdate())=1 then 12 else MONTH(getdate())-1 end) end
and AxProductId in (20005,20997,20999,20995,20992,20993)
...
(
select distinct UserName,AccountId,NetworkId from Fills
where YEAR=year(getdate()) and MONTH=month(GETDATE()) and AxProductId in (20005,20997,20999,20995,20992,20993)
and IsBillable='Y' and NetworkId not in (577, 624)
)A
left join 
(
...
(
select distinct UserName,AccountId,NetworkId from Fills
where YEAR=case when month(GETDATE())=1 then year(getdate())-1 else year(getdate()) end and MONTH=case when month(GETDATE())=1 then 12 else month(GETDATE())-1 end  and AxProductId in (20005,20997,20999,20995,20992,20993)
and IsBillable='Y' and NetworkId not in (577, 624)
)A
left join 
(
...
when month(getdate())=1 then 11 
when month(getdate())=2 then 12
else month(GETDATE())-2 end  and AxProductId in (20005,20997,20999,20995,20992,20993)
and IsBillable='Y' and NetworkId not in (577, 624)
)A
left join 
(
...
when month(getdate())=2 then 11
when month(getdate())=3 then 12
else month(GETDATE())-3 end and AxProductId in (20005,20997,20999,20995,20992,20993)
and IsBillable='Y' and NetworkId not in (577, 624)
)A
left join 
(

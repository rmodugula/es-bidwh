...
select distinct ProductName
  FROM [BIDW].[dbo].[Fills]
  where MarketId = 90 and year = 2017 and month = 9
  and IsBillable = 'Y'  
  and ProductName not in 
  (
  select distinct ProductName
  FROM [BIDW].[dbo].[Fills]
  where MarketId = 90 --and year = 2017 and month < 5
  and IsBillable = 'Y'
...
FROM ( SELECT Year,Month,SUM(fills) AS fills , UserName , AccountId , [FillCategoryId]--,[DayofMonth]
       ...
       WHERE year = 2017
             ...
             IsBillable = 'Y'
             AND
             UserName IN ('ADOMICIANO' , 'FELIPE' , 'RMENDES','GARDEEQUITY','LCOSSI'
                         )
       --and username IN ('ADOMICIANO','RTADEWALD') 
